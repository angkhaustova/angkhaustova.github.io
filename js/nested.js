jQuery(document).ready(function ($) {
    ...
    $('.toggle-animation').click(function () {
        $('[data-view=editor-view]').each(function () {
            toggleView($(this));
        });
        ...
        var allEls = $('*');
        allEls.removeClass('pale');
        allEls.removeClass('bright');
        allEls.removeClass('go-active');
        allEls.removeClass('inscribed-view-init');
        //$('.inscribed-view').hide();
        //$('.inscribed').show();
        allEls.removeClass('inscribed-view');
        allEls.removeClass('crossed-view-init');
        allEls.removeClass('crossed-view');
    });
    ...
    $('.complexFragmentMarker').click(function () {
        ...
        activeFragment.find('.inscribed').addClass('inscribed-view-init');
...
function analyzeText(text) {
    ...
    while (opening !== null || closing !== null) {
        if (opening === null || opening.index > closing.index) {
            ...
            sub = '<span id="' + id + '" class="' + spanClass + '" data-lvl="' + opened.length + '">';
    ...
    newText = newText.replace(/([{}])/g, '<span class="brace">$1</span>');
    newText = newText.replace(/([\[\]])/g, '<span class="bracket">$1</span>');
    newText = newText.replace(/(&lt;|&gt;)/g, '<span class="chevron">$1</span>');
